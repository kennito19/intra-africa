...
export const checkCase = (card) => {
  ...
  switch (card?.redirect_to) {
    case 'Product list':
      ...
      if (card?.categoryId) {
        url += `/products/${
          spaceToDash(card?.categoryName) ?? 'products'
        }?CategoryId=${encryptId(card?.categoryId)}`
      }
      if (card?.brandIds) {
        url += `&BrandIds=${encryptId(card?.brandIds)}`
      }
      if (card?.sizeids) {
        url += `&SizeIds=${encryptId(card?.sizeids)}`
      }
      if (card?.colorIds) {
        url += `&ColorIds=${encryptId(card?.colorIds)}`
      }
      break

    case 'Specific product':
      url = `/product/${
        spaceToDash(card?.productName) ?? 'product'
      }/${encryptId(card?.productId)}`
      break
    ...
    case 'Landing page':
      url = `/landing-page?LendingPageId=${encryptId(card?.lendingPageId)}`
      break
...
export const handleToken = async ({
  ...
  if (
    ...
    if (getNewToken?.data?.code === 200) {
      const newAccessToken = await getNewToken?.data?.accessToken
      const newRefreshToken = await getNewToken?.data?.refreshToken
      Nookies.set(ctx, 'userToken', newAccessToken, {
        path: '/'
      })
      Nookies.set(ctx, 'refreshToken', newRefreshToken, {
        path: '/'
      })
      try {
        ...
        const req = await axios.get(`${baseUrl}${apiUrl}`, newConfig)
        if (req?.data?.code === 200 || req?.data?.code === 204) {
          return { response: req?.data, tokenType }
        } else {
          throw error
        }
      } catch (error) {
    ...
    tokenType = 'noAuth'
    // Get No Auth Token
    const getNoAuth = await axios.get(
      `${baseUrl}Account/GenerateNoAuthToken?deviceId=${deviceId}`
    )
    Nookies.set(ctx, 'userToken', getNoAuth?.data?.Token, {
      path: '/'
    })
    Nookies.set(ctx, 'deviceId', deviceId, {
      path: '/'
    })
    try {
      let config = {
        headers: {
          ...
          accept: '*/*',
          device_id: deviceId,
          Authorization: `Bearer ${getNoAuth?.data?.Token ?? ''}`
        }
      }
      const req = await axios.get(`${baseUrl}${apiUrl}`, config)
      if (req?.data?.code === 200 || req?.data?.code === 204) {
        return { response: req?.data, tokenType }
      } else {
        throw error
...
//       continue // Skip the "specifications" key
//     }
//     if (obj.hasOwnProperty(key) && obj[key] !== null) {
//       if (Array.isArray(obj[key])) {
//         queryString.push(`${key}=${obj[key].join(',')}`)
//       } else {
  ...
  for (const key in obj) {
    ...
    if (obj.hasOwnProperty(key) && obj[key] !== null) {
      ...
      if (
        ...
        encodedValue = objKey?.map((data) =>
          isValueToDecode ? decryptId(data) : encryptId(data)
        )
        ...
        encodedValue = isValueToDecode ? obj[key] : encryptId(obj[key])
      } else {
        encodedValue = isValueToDecode
          ? obj[key].includes(',')
            ? (obj[key] || '')
            ...
            : key === 'categoryName'
            ? obj[key]
            : typeof decryptId(obj[key]) === 'string'
            ? decryptId(obj[key])
            : null
          : encryptId(obj[key])
      }
...
export function formatSpecifications(specifications) {
  ...
  if (Array.isArray(specifications)) {
    specifications?.forEach((item) => {
      ...
      if (specMap[specId]) {
        specMap[specId].push(encryptId(value))
      } else {
        specMap[specId] = [encryptId(value)]
      }
    })
...
export const spaceToDash = (inputString, dashtoSpace = false) => {
  ...
  if (dashtoSpace) {
    stringWithoutTrailingDash = inputString.replace(/-/g, ' ')
  } else {
    const stringWithoutSpaces = inputString?.replace(/[\/\s]+/g, '-')
    stringWithoutTrailingDash = stringWithoutSpaces?.replace(/-$/, '')
  }
